pipeline {
  ...
  stage("Quality Gate"){
    ...
    stage("Trivy Scan"){
      steps {
        script {
          sh ('docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy Image shariq81/register-app-pipeline:latest -q --scanners vuln --exit-code 0 --severity HIGH,CRITICAL --format table')
        }
      }
    }
