pipeline {
  ...
  stage("Quality Gate"){
    ...
    stage("Trivy Scan"){
      steps {
        script {
                sh ('docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy image shariq81/register-app-pipeline:latest -q --severity HIGH,CRITICAL --format table')
          }
        }
      }
