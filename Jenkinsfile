pipeline {
  ...
  stage("Quality Gate"){
    ...
    stage("Trivy Scan"){
      steps {
        script {
                sh ('docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy shariq81/register-app-pipeline:latest -q --exit-code 0 --severity HIGH,CRITICAL --format table')
          }
        }
      }
